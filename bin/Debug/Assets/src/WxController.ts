        ...
        this.wx.login({
            ...
            fail: function () {
                ...
                if (this.retryCount > 3) {//超时三次
                    that.wx.showModal({
                        title: localization.string_8,
                        content: localization.string_9,
                        cancelText: localization.string_10,
                        confirmText: localization.string_11,
                        success: (res) => {
                            if (res.confirm) {
                                //重试
                                that.login(callback);
    ...
    */
    ...
    public showFriendRanking(cmd: SubContextCMD, container: Laya.Sprite) {
        ...
        var ontimeoutF = function () {
            that.wx.hideLoading({});
            that.wx.showModal({
                title: localization.string_12,
                content: localization.string_13,
                cancelText: localization.string_14,
                confirmText: localization.string_15,
                success: (res) => {
                    if (res.confirm) {
                        //重试
                        retryFun();
